...
def find_closest_skin_tone(detected_rgb):
    """Finding the closest matching skin tone from the dataset."""
...
def detect_skin_tone_with_landmarks(image):
    """Detecting skin tone using Mediapipe face landmarks."""
    ...
    # Extracting landmarks for skin tone analysis
    skin_regions = []
...
def generate_seasonal_palette(season, base_rgb, global_used_colors, num_colors=15, min_diff=50):
    """Generating diverse seasonal palettes with global uniqueness."""
    ...
    # Converting base color to HSV
    r, g, b = [x / 255.0 for x in base_rgb]
    h, s, v = rgb_to_hsv(r, g, b)
    palette = []
    used_colors = set()

    # Retrieving seasonal adjustments
    adjustments = seasonal_adjustments.get(season, {})

    for _ in range(num_colors * 20):  # Ensuring enough attempts for diverse colors
        ...
        # Ensuring the color is unique globally and within the current palette
        if (
        ...
        if len(palette) >= num_colors:  # Stopping once the required number of colors is generated
            break
...
def recommend_makeup(detected_rgb, category, limit=5):
    """Recommending makeup shades based on detected skin tone."""
...
# File uploading or camera input
...
if image_file:
    ...
    if detected_rgb is not None:
        ...
        with tab2:
            ...
            # Generating and displaying palette
        ...
        if show_makeup == "Yes":
            # Displaying makeup recommendations
